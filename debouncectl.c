...
static int enumerate_devices(device_info *devs, int max) {
    ...
    char excluded[64][PATH_MAX]; int exclude_count;
...
int main(int argc, char *argv[]) {
    ...
    if(strcmp(argv[1],"stop")==0 || strcmp(argv[1],"show")==0) {
        ...
        if(argc>5) { fprintf(stderr,"Too many arguments with device path\n"); print_usage(argv[0]); return 1; }
        dev_idx = 1;
    } else {
